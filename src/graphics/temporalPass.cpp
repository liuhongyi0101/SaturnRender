...
void TemporalPass::createFramebuffersAndRenderPass(uint32_t width, uint32_t  height)
{
...
void TemporalPass::createDescriptorsLayouts(VkDescriptorPool &descriptorPool)
{
...
void TemporalPass::createPipeline()
{
	...
	shaderStages[1] = loadShader(getAssetPath + std::string("taa/taa.frag.spv"), VK_SHADER_STAGE_FRAGMENT_BIT, device, shaderModules);

	VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, 0, 1, &pipelineCreateInfo, nullptr, &pipeline));

}
void TemporalPass::wirteDescriptorSets()
{
	writeDescriptorSets = {
...
void TemporalPass::createUniformBuffers(VkQueue queue, glm::mat4 &invPerspective, glm::vec3 &cameraPos)
{
	// Scene matrices
	...
	glm::mat4 inpv = glm::mat4(1.0);
	updateUniformBufferMatrices(inpv);
}

void TemporalPass::updateUniformBufferMatrices(glm::mat4 &invPerspective)
{

	uboParams.invpv = invPerspective;
	VK_CHECK_RESULT(uniformBuffers.map());
	uniformBuffers.copyTo(&uboParams, sizeof(uboParams));
	uniformBuffers.unmap();
}
void TemporalPass::buildCommandBuffer(VkCommandBuffer &cmdBuffer)
{
